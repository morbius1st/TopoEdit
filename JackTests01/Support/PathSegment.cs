...
namespace JackTests01.Support
{
	...
	public class PathSegment : INotifyPropertyChanged
	{
		...
		private PathSegment priorSegment;
	...
	#region construction
		...
		{
			PathSegment ps = new PathSegment(PathSegmentType.PST_ROOT, "root");
			ps.priorSegment = ps;
			lastSegment = ps;
			index = 0;
			index++;
		...
		{
			PathSegment ps =  new PathSegment(PathSegmentType.PST_TERM, "termination");
	...
	#region public methods
		...
		public void UpdateParent()
		{
			foreach (SegmentPoint psd in SegmentData)
			{
				...
				if (ListBoxData01.DbSwitch)
				{
					Debug.WriteLine($"for| {psd.PointType} | {psd.XyzCoordFmtd} | {psd.PriorFmtd}");
				}
			}
		}
	...
	#region overrides
		...
		{
			string count = "0";
			string first = @"invalid";

			if (SegmentData != null)
			{
				count = $"{SegmentData.Count}";

				first = SegmentData.Count > 0 ? FormatNumber.fmtXyz(SegmentData[0].LocPoint) : "empty";
			}

			return $"{seq,-3} | {SegmentNum,-10} | {SegTypeDesc,-20} | count| {count} | {first}";
		}
